...
recovery_opkg_settings() {
	...
	*3000*)
		...
		mt3000_opkg="${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/mt-3000/distfeeds.conf"
		wget -O /etc/opkg/distfeeds.conf ${mt3000_opkg}
		;;
	*2500*)
		echo "Router name contains '2500'."
		mt2500a_opkg="${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/mt-2500a/distfeeds.conf"
...
update_opkg_config() {
	...
	5.4*)
		mt6000_opkg="${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/mt-6000/distfeeds-5.4.conf"
		wget -O /etc/opkg/distfeeds.conf ${mt6000_opkg}
		# 更换5.4.238 内核之后 缺少的依赖

		mkdir -p /tmp/mt6000
		wget -O /tmp/mt6000/script-utils.ipk "${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/mt-6000/script-utils.ipk?$(date +%s)"
		wget -O /tmp/mt6000/mdadm.ipk "${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/mt-6000/mdadm.ipk?$(date +%s)"
		wget -O /tmp/mt6000/lsblk.ipk "${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/mt-6000/lsblk.ipk?$(date +%s)"
		opkg update
		...
		;;
	5.15*)
		mt6000_opkg="${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/mt-6000/distfeeds.conf"
		wget -O /etc/opkg/distfeeds.conf ${mt6000_opkg}
		;;
	*)
...
do_install_filetransfer() {
	...
	wget -O luci-app-filetransfer_all.ipk "${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/luci-app-filetransfer/luci-app-filetransfer_all.ipk"
	wget -O luci-lib-fs_1.0-14_all.ipk "${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/luci-app-filetransfer/luci-lib-fs_1.0-14_all.ipk"
	opkg install *.ipk --force-depends
}
do_install_depends_ipk() {

	wget -O "/tmp/luci-lua-runtime_all.ipk" "${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/theme/luci-lua-runtime_all.ipk"
	wget -O "/tmp/libopenssl3.ipk" "${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/theme/libopenssl3.ipk"
...
do_install_argon_skin() {
	...
	wget -O "/tmp/luci-theme-argon.ipk" "${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/theme/luci-theme-argon-master_2.2.9.4_all.ipk"
	wget -O "/tmp/luci-app-argon-config.ipk" "${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/theme/luci-app-argon-config_0.9_all.ipk"
	wget -O "/tmp/luci-i18n-argon-config-zh-cn.ipk" "${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/theme/luci-i18n-argon-config-zh-cn.ipk"
	cd /tmp/
...
#更新脚本
update_myself() {
	wget -O gl-inet.sh ${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/gl-inet.sh?$(date +%s) && chmod +x gl-inet.sh
...
mt3000_overlay_changed(){
	wget -O mt3000.sh ${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/mt-3000/mt3000.sh && chmod +x mt3000.sh
	sh mt3000.sh
}
...
while true; do
	...
	11)
		...
		read -r answer
		if [ "$answer" = "y" ] || [ -z "$answer" ]; then
			wget -q -O do_docker.sh ${proxy}https://raw.githubusercontent.com/lautk/gl-inet-onescript/master/docker/do_docker.sh && chmod +x do_docker.sh
			if [ -z "$proxy" ]; then
				./do_docker.sh
			else
